...
from .torch_utils import transposed_matmul, maybe_remat
...
class Downsample(nn.Module):
    def __init__(self, c_in, c_out, strides):
        ...
        if self.has_conv:
            self.conv_layer = Conv2D(c_in, c_out, 3)
...
class Upsample(nn.Module):
    ...
    def forward(self, x, label=None):
        B, C, H, W = x.shape
        x = F.interpolate(x, size=[H * self.strides, W * self.strides], mode="area")
        if self.has_conv:
            x = self.conv_layer(x)
        return x

class ResBlock(nn.Module):
    def __init__(self, c, c_out, c_in=None, w_scale=1.0, sr_lam=None, num_classes=0, is_conv=True, checkpoint=False):
        super().__init__()
        self.c = c
        self.c_out = c_out
        self.c_in = c_in or c
        ...
        if self.num_classes > 0:
            self.label_emb = nn.Embedding(self.num_classes+1, self.c//4)
            self.label_proj = nn.Linear(self.c//4, self.c_in)
    ...
    def forward(self, inputs, label=None):
        ...
        if self.num_classes > 0:
            label = self.label_proj(self.label_emb(label))
            x += label[:, :, None, None]
